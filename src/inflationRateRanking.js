...
const inflationRateDataUrl = new URL("../data/weo/current-prices.json", import.meta.url);
const rankingEndYear = 2026;
const rankingTitleBase = "Inflation Rate Ranking";
...
initializeRanking().catch((error) => {
  console.error("[Ranking] Failed to initialize inflation rate ranking.", error);
  showRankingError();
});
...
async function initializeRanking() {
  ...
  const response = await fetch(inflationRateDataUrl, {
    headers: {
  ...
  if (!response.ok) {
    throw new Error(`Static inflation rate data file request failed: ${response.status} ${response.statusText}`);
  }

  const data = await response.json();
  allRankingRows = buildInflationRateRanking(data);
  renderScopedRanking();
}

function buildInflationRateRanking(data) {
  const inflationRateByCountry = getIndicatorSeriesMap(data, "PCPIPCH");

  if (!inflationRateByCountry || typeof inflationRateByCountry !== "object") {
    throw new Error("Static inflation rate data file is missing PCPIPCH series.");
  }

  return countries
    .filter((country) => country.includeInRankings !== false)
    .map((country) => {
      const latestPoint = getLatestNumericPoint(inflationRateByCountry[country.code]);
...
function renderRankingTable(rankingRows) {
  ...
  rankingRows.forEach((country, index) => {
    ...
    valueLink.href = `${rootHref}countries/${country.slug}/inflation-rate/`;
    valueLink.textContent = formatDisplayValue(country.value, displayScale);
    valueLink.setAttribute("aria-label", `Open ${country.name} Inflation Rate page`);
