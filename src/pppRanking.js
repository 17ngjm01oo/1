...
function renderRankingTable(rankingRows) {
  ...
  rankingRows.forEach((country, index) => {
    ...
    valueLink.href = `${rootHref}countries/${country.slug}/ppp/`;
    valueLink.textContent = formatCompactRankingValue(country.value, displayScale);
    valueLink.setAttribute("aria-label", `Open ${country.name} PPP page`);
...
function updateRankingSummary(rankingRows, scope) {
  ...
  rankingSummary.textContent =
    `Click a value to open that country's PPP chart. ${rankingRows.length} countries shown for ${scopeLabel}.`;
}
