...
const populationDataUrl = new URL("../data/weo/current-prices.json", import.meta.url);
const rankingEndYear = 2026;
const rankingTitleBase = "Population Ranking";
...
initializeRanking().catch((error) => {
  console.error("[Ranking] Failed to initialize population ranking.", error);
  showRankingError();
});
...
async function initializeRanking() {
  ...
  if (!response.ok) {
    throw new Error(`Static population data file request failed: ${response.status} ${response.statusText}`);
  }

  const data = await response.json();
  allRankingRows = buildPopulationRanking(data);
  renderScopedRanking();
}

function buildPopulationRanking(data) {
  const populationByCountry = getIndicatorSeriesMap(data, "LP");

  if (!populationByCountry || typeof populationByCountry !== "object") {
    throw new Error("Static population data file is missing LP series.");
  }

  return countries
    .filter((country) => country.includeInRankings !== false)
    .map((country) => {
      const latestPoint = getLatestNumericPoint(populationByCountry[country.code]);
...
function renderRankingTable(rankingRows) {
  ...
  rankingRows.forEach((country, index) => {
    ...
    appendRankingValueCell(valueCell, {
      href: `${rootHref}countries/${country.slug}/population/`,
      text: formatCompactDisplayValue(country.value, displayScale),
      ariaLabel: `Open ${country.name} Population page`,
      value: country.value,
      rankingRows,
    });
