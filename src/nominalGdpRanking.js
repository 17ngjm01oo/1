...
async function initializeRanking() {
  ...
  if (!response.ok) {
    throw new Error(`Static GDP data file request failed: ${response.status} ${response.statusText}`);
  }
...
function buildNominalGdpRanking(data) {
  ...
  return countries
    ...
    .map((country) => {
      const latestPoint = getLatestNumericPoint(gdpByCountry[country.code]);
